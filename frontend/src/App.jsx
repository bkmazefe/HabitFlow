...
import { Dashboard, Habits, Tracker, Reports, Settings, Login, Register, ForgotPassword } from './pages'
...
function App() {
  ...
  // Backend ile değişecek - Habits verisi backend API'den çekilecek
  const [habits, setHabits] = useState(() => {
    const saved = localStorage.getItem('habits')
    if (saved) return JSON.parse(saved)
    // Backend ile değişecek - Varsayılan habit'ler kaldırılacak, kullanıcının verileri gelecek
    return [
      {
        id: 1,
  ...
  const renderPage = () => {
    switch (currentPage) {
      ...
      case 'dashboard':
        return <Dashboard habits={habitsWithCompletion} onLogValue={handleLogValue} t={t} />
      case 'habits':
        return (
          <Habits
      ...
      case 'settings':
        return <Settings theme={theme} onThemeChange={setTheme} language={language} onLanguageChange={setLanguage} t={t} />
      default:
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gradient-to-br from-white to-gray-50 dark:from-slate-900 dark:to-slate-800 transition-colors duration-300">
      {!isAuthPage && <Navigation currentPage={currentPage} setCurrentPage={setCurrentPage} t={t} />}
      <main className="flex-grow">{renderPage()}</main>
      {!isAuthPage && <Footer t={t} onNavigate={setCurrentPage} currentPage={currentPage} />}
    </div>
  )
}
