...
export default function HabitForm({ habit, onSubmit, onCancel, t }) {
  ...
  // Backend ile değişecek - POST veya PUT /api/habits endpoint'ine istek atılacak
  const handleSubmit = (e) => {
    e.preventDefault()
    if (formData.name.trim()) {
      // Backend ile değişecek - API response'u beklenecek
      onSubmit({
        ...formData,
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      {formData.unit !== 'none' && (
        <div>
          <label className="block text-sm font-semibold text-gray-700 dark:text-slate-300 mb-1">
            {t?.habitForm?.target || 'Target'} ({formData.frequency === 'daily' ? (t?.habitForm?.dailyTarget || 'Daily Target') : (t?.habitForm?.weeklyTarget || 'Weekly Target')})
          </label>
          <input
            type="number"
            name="targetValue"
          ...
          <p className="text-xs text-gray-500 dark:text-slate-400 mt-1">
            {formData.unit === 'minutes' && `${formData.targetValue} ${t?.habitForm?.minutes || 'minutes'} ${formData.frequency === 'daily' ? (t?.common?.perDay || 'per day') : (t?.common?.perWeek || 'per week')}`}
            {formData.unit === 'pages' && `${formData.targetValue} ${t?.habitForm?.pages || 'pages'} ${formData.frequency === 'daily' ? (t?.common?.perDay || 'per day') : (t?.common?.perWeek || 'per week')}`}
            {formData.unit === 'count' && `${formData.targetValue} ${t?.habitForm?.count || 'count'} ${formData.frequency === 'daily' ? (t?.common?.perDay || 'per day') : (t?.common?.perWeek || 'per week')}`}
            {formData.unit === 'hours' && `${formData.targetValue} ${t?.habitForm?.hours || 'hours'} ${formData.frequency === 'daily' ? (t?.common?.perDay || 'per day') : (t?.common?.perWeek || 'per week')}`}
            {formData.unit === 'times' && `${formData.targetValue} ${t?.habitForm?.times || 'times'} ${formData.frequency === 'daily' ? (t?.common?.perDay || 'per day') : (t?.common?.perWeek || 'per week')}`}
          </p>
        </div>
      )}
      ...
      <div className="flex gap-3 pt-4">
        ...
        >
          {habit ? (t?.habitForm?.updateHabit || 'Update Habit') : (t?.habitForm?.createHabit || 'Create Habit')}
        </button>
        <button
          type="button"
          onClick={onCancel}
