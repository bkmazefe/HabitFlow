...
export default function Reports({ habits, t }) {
  ...
  // Backend ile değişecek - Kategori istatistikleri GET /api/stats/categories endpoint'inden çekilecek
  const categoryData = [
  ...
  return (
    <div className="w-full bg-white dark:bg-transparent transition-colors duration-300">
      <div className="max-w-6xl mx-auto px-4 py-8">
        <h1 className="text-4xl font-bold text-[#99BBE2] mb-2">{t?.reports?.title || 'Reports & Analytics'}</h1>
        <p className="text-gray-600 dark:text-slate-400 mb-8">{t?.reports?.subtitle || 'Track your progress and see what works for you'}</p>

        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          {/* Habit Performance */}
          <ProgressChart data={monthlyData} title={t?.reports?.habitPerformance || 'Habit Performance'} />

          {/* Category Analytics */}
          <ProgressChart data={categoryData} title={t?.reports?.categoryProgress || 'Category Progress'} />
        </div>
        ...
        <div className="mt-8 bg-white dark:bg-slate-800/50 dark:backdrop-blur-sm rounded-xl shadow-soft dark:shadow-slate-900/30 p-6 transition-colors duration-300 dark:border dark:border-slate-700/50">
          <h3 className="text-2xl font-bold text-[#99BBE2] mb-6">{t?.reports?.detailedStats || 'Detailed Statistics'}</h3>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="p-4 bg-[#F2AFC1] dark:bg-[#F2AFC1]/15 rounded-lg text-center">
              <p className="text-gray-600 dark:text-slate-300 font-semibold mb-2">{t?.reports?.totalCheckins || 'Total Check-ins'}</p>
            ...
            <div className="p-4 bg-[#D7C8F3] dark:bg-[#D7C8F3]/15 rounded-lg text-center">
              <p className="text-gray-600 dark:text-slate-300 font-semibold mb-2">{t?.reports?.avgDailyRate || 'Avg Daily Rate'}</p>
              <p className="text-3xl font-bold text-[#99BBE2]">
                {habits.length > 0
                  ? Math.round(
                      (habits.reduce((sum, h) => sum + (h.completedToday ? 1 : 0), 0) /
            ...
            <div className="p-4 bg-[#F2AFC1] dark:bg-[#F2AFC1]/15 rounded-lg text-center">
              <p className="text-gray-600 dark:text-slate-300 font-semibold mb-2">{t?.reports?.bestStreak || 'Best Streak'}</p>
              <p className="text-3xl font-bold text-[#99BBE2]">
                {Math.max(...habits.map((h) => h.streak), 0)} {t?.common?.days || 'days'}
              </p>
            </div>
          </div>
        </div>
        ...
        {/* Export Section */}
        <div className="mt-8 bg-white dark:bg-slate-800/50 dark:backdrop-blur-sm rounded-xl shadow-soft dark:shadow-slate-900/30 p-6 transition-colors duration-300 dark:border dark:border-slate-700/50">
          <h3 className="text-xl font-bold text-[#99BBE2] mb-4">{t?.reports?.exportData || 'Export Data'}</h3>
