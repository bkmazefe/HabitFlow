    ...
    <div className="w-full bg-white dark:bg-transparent transition-colors duration-300">
      <div className="max-w-6xl mx-auto px-4 py-8">
        <h1 className="text-4xl font-bold text-[#99BBE2] mb-2">
          {t?.tracker?.title || "Daily Tracker"}
        </h1>
        <p className="text-gray-600 dark:text-slate-400 mb-8">
          {t?.tracker?.subtitle || "Log your progress for today"}
        </p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1 bg-white dark:bg-slate-800/50 dark:backdrop-blur-sm rounded-xl shadow-md dark:shadow-slate-900/30 p-6 transition-colors duration-300 dark:border dark:border-slate-700/50">
            ...
            <div className="grid grid-cols-7 gap-1 mb-2">
              {["S", "M", "T", "W", "T", "F", "S"].map((day, i) => (
                <div
                  key={i}
                  className="text-center font-semibold text-[#99BBE2] text-xs py-1"
                >
                  {day}
                </div>
              ))}
          ...
          <div className="lg:col-span-2 bg-white dark:bg-slate-800/50 dark:backdrop-blur-sm rounded-xl shadow-md dark:shadow-slate-900/30 p-6 transition-colors duration-300 dark:border dark:border-slate-700/50">
            <h3 className="text-xl font-bold text-[#99BBE2] mb-4">
              📝 {t?.tracker?.logProgress || "Log Today's Progress"}
            </h3>
            ...
            {habits.length > 0 ? (
                ...
                {habits.map((habit) => (
                  ...
                  >
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-500 dark:text-slate-400">
                          {habit.frequency === "daily"
                            ? t?.habits?.daily || "Daily"
                            : t?.habits?.weekly || "Weekly"}{" "}
                          {t?.habits?.target || "target"}: {habit.targetValue}{" "}
                          {UNIT_LABELS[habit.unit]}
                        </p>
                      </div>
                      {habit.completedToday && (
                    ...
                    {habit.unit === "none" ? (
                      ...
                      <button
                        onClick={() =>
                          onToggleHabit(habit.id, !habit.completedToday)
                        }
                      ...
                      >
                        {habit.completedToday
                          ? `✓ ${t?.tracker?.completed || "Completed"}`
                          : t?.tracker?.markComplete || "Mark Complete"}
                      </button>
                    ) : (
                      // Değer girişi
                        ...
                        <div className="flex-1">
                          <div className="flex items-center justify-center gap-2 mb-1">
                            <input
                              ...
                              onChange={(e) =>
                                onLogValue(habit.id, e.target.value)
                              }
                            ...
                            <span className="text-gray-600 dark:text-gray-300">
                              / {habit.targetValue} {UNIT_LABELS[habit.unit]}
                            </span>
                          </div>
                          {/* Progress bar */}
                          <div className="w-full bg-gray-200 dark:bg-gray-600 rounded-full h-2">
                            <div
                              ...
                              style={{
                                width: `${Math.min(habit.completion, 100)}%`,
                              }}
                            />
                          </div>
                          <p className="text-center text-xs text-gray-500 dark:text-gray-400 mt-1">
                            {habit.completion}%{" "}
                            {t?.tracker?.completedLower || "completed"}
                          </p>
                        </div>
              ...
              <p className="text-gray-500 dark:text-gray-400 text-center py-8">
                {t?.tracker?.noHabits || "No habits to track yet!"}
              </p>
            )}
          </div>
        </div>
