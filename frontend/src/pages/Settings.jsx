...
import { FiUser, FiSettings, FiCamera, FiMail, FiPhone, FiMapPin, FiCalendar, FiEdit2, FiSun, FiMoon } from 'react-icons/fi'

export default function Settings({ theme, onThemeChange, language, onLanguageChange, t }) {
  ...
  // Backend ile değişecek - Kullanıcı profil bilgileri GET /api/user/profile endpoint'inden çekilecek
  ...
  // Backend ile değişecek - PUT /api/user/profile endpoint'ine istek atılacak
  const handleSaveProfile = () => {
    setIsEditingProfile(false)
    // Backend ile değişecek - Burada backend'e kaydetme yapılacak
  }
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="flex items-center justify-between mb-8">
          <h1 className="text-4xl font-bold text-[#99BBE2]">
            {activeTab === 'profile' ? (t?.settings?.profile || 'Profile') : (t?.settings?.settings || 'Settings')}
          </h1>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-gradient-to-r from-[#99BBE2]/20 via-[#D7C8F3]/20 to-[#F2AFC1]/20 dark:from-[#99BBE2]/10 dark:via-[#D7C8F3]/10 dark:to-[#F2AFC1]/10 rounded-2xl p-8">
              <div className="flex flex-col md:flex-row items-center gap-6">
                ...
                >
                  <FiEdit2 size={18} />
                  {isEditingProfile ? (t?.settings?.cancel || 'Cancel') : (t?.settings?.editProfile || 'Edit Profile')}
                </button>
              </div>
            </div>
            ...
            <div className="bg-white dark:bg-slate-800/50 dark:backdrop-blur-sm rounded-xl shadow-soft dark:shadow-slate-900/30 p-6 dark:border dark:border-slate-700/50">
              <h3 className="text-xl font-bold text-[#99BBE2] mb-6">{t?.settings?.personalInfo || 'Personal Information'}</h3>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  {isEditingProfile ? (
                    ...
                    <p className="px-4 py-3 bg-gray-50 dark:bg-slate-700/50 rounded-xl text-gray-800 dark:text-slate-200">{profile.name}</p>
                  )}
                </div>
                ...
                <div>
                  ...
                  {isEditingProfile ? (
                    ...
                    <p className="px-4 py-3 bg-gray-50 dark:bg-gray-700 rounded-xl text-gray-800 dark:text-white">{profile.email}</p>
                  )}
                </div>
                ...
                <div>
                  ...
                  {isEditingProfile ? (
                    ...
                    <p className="px-4 py-3 bg-gray-50 dark:bg-slate-700/50 rounded-xl text-gray-800 dark:text-slate-200">{profile.phone}</p>
                  )}
                </div>
                ...
                <div>
                  ...
                  {isEditingProfile ? (
                    ...
                    <p className="px-4 py-3 bg-gray-50 dark:bg-slate-700/50 rounded-xl text-gray-800 dark:text-slate-200">{profile.location}</p>
                  )}
                </div>
                ...
                <div className="md:col-span-2">
                  ...
                  {isEditingProfile ? (
                    ...
                    <p className="px-4 py-3 bg-gray-50 dark:bg-slate-700/50 rounded-xl text-gray-800 dark:text-slate-200">{profile.bio}</p>
                  )}
                </div>
              </div>
        ...
        {activeTab === 'settings' && (
          <div className="space-y-6">
            {/* Notifications */}
            <div className="bg-white dark:bg-slate-800/50 dark:backdrop-blur-sm rounded-xl shadow-soft dark:shadow-slate-900/30 p-6 dark:border dark:border-slate-700/50">
              <h3 className="text-xl font-bold text-[#99BBE2] mb-4">{t?.settings?.notifications || 'Notifications'}</h3>
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between p-4 bg-gradient-to-r from-[#F2AFC1]/20 to-[#F2AFC1]/10 dark:from-[#F2AFC1]/15 dark:to-transparent rounded-xl">
                  <label className="font-semibold text-gray-800 dark:text-slate-200">{t?.settings?.pushNotifications || 'Push Notifications'}</label>
                ...
                <div className="flex items-center justify-between p-4 bg-gradient-to-r from-[#D7C8F3]/20 to-[#D7C8F3]/10 dark:from-[#D7C8F3]/15 dark:to-transparent rounded-xl">
                  <label className="font-semibold text-gray-800 dark:text-slate-200">{t?.settings?.emailReminders || 'Email Reminders'}</label>
                  <label className="relative inline-flex items-center cursor-pointer">
                    <input
                      type="checkbox"
            ...
            {/* Preferences */}
            <div className="bg-white dark:bg-slate-800/50 dark:backdrop-blur-sm rounded-xl shadow-soft dark:shadow-slate-900/30 p-6 dark:border dark:border-slate-700/50">
              <h3 className="text-xl font-bold text-[#99BBE2] mb-4">{t?.settings?.preferences || 'Preferences'}</h3>

              <div className="space-y-4">
                <div>
                  <label className="block font-semibold text-gray-700 dark:text-slate-300 mb-3">{t?.settings?.theme || 'Theme'}</label>
                ...
                <div>
                  <label className="block font-semibold text-gray-700 dark:text-slate-300 mb-2">{t?.settings?.language || 'Language'}</label>
                  <select
            ...
            {/* Account */}
            <div className="bg-white dark:bg-slate-800/50 dark:backdrop-blur-sm rounded-xl shadow-soft dark:shadow-slate-900/30 p-6 dark:border dark:border-slate-700/50">
              <h3 className="text-xl font-bold text-[#99BBE2] mb-4">{t?.settings?.account || 'Account'}</h3>
            ...
            {/* About */}
            <div className="bg-white dark:bg-slate-800/50 dark:backdrop-blur-sm rounded-xl shadow-soft dark:shadow-slate-900/30 p-6 dark:border dark:border-slate-700/50">
              <h3 className="text-xl font-bold text-[#99BBE2] mb-4">{t?.settings?.about || 'About'}</h3>
              <div className="space-y-2 text-gray-700 dark:text-slate-400">
                <p><strong className="dark:text-slate-200">{t?.settings?.appVersion || 'App Version'}:</strong> 1.0.0</p>
                <p><strong className="dark:text-slate-200">{t?.settings?.buildDate || 'Build Date'}:</strong> December 2025</p>
                <p><strong className="dark:text-slate-200">{t?.settings?.supportEmail || 'Support Email'}:</strong> support@habitflow.com</p>
              </div>
            </div>
          </div>
        )}
