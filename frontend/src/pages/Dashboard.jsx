...
import { FiTarget, FiCheckCircle, FiTrendingUp, FiBarChart2, FiPlay, FiPause, FiSquare } from 'react-icons/fi'
...
export default function Dashboard({ habits, onLogValue, t }) {
  ...
  const dailyQuote = quotes[quoteIndex] || { text: "Small steps lead to big changes.", author: "Kaizen Philosophy" }
  ...
  const timeBasedHabits = habits.filter(h => h.unit === 'minutes' || h.unit === 'hours')
  ...
  useEffect(() => {
    ...
    if (!isFreeTimer && selectedHabitId && (selectedHabitId !== prevSelectedHabitIdRef.current)) {
  ...
  // Backend ile değişecek - onLogValue POST /api/habits/:id/log endpoint'ine istek atacak
  const handleStop = () => {
    setIsRunning(false)
  ...
  const avgCompletion =
    habits.length > 0
      ? Math.round(habits.reduce((sum, h) => sum + h.completion, 0) / habits.length)
      : 0
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div className="bg-white dark:bg-slate-800/50 dark:backdrop-blur-sm rounded-xl shadow-md dark:shadow-slate-900/30 p-6 h-full transition-colors duration-300 dark:border dark:border-slate-700/50">
            <ProgressChart data={progressData} title={t?.dashboard?.habitCompletion || 'Habit Completion Rate'} />
          </div>
          
          <div className="bg-white dark:bg-slate-800/50 dark:backdrop-blur-sm rounded-xl shadow-md dark:shadow-slate-900/30 p-6 h-full transition-colors duration-300 dark:border dark:border-slate-700/50">
            <h3 className="text-lg font-bold text-gray-800 dark:text-slate-100 mb-4">{t?.dashboard?.thisWeek || 'This Week'}</h3>
            <div className="space-y-3">
              {(t?.dashboard?.weekDays || ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun']).map((day, i) => (
                <div key={day} className="flex items-center gap-3">
                  <span className="font-semibold text-gray-600 dark:text-slate-400 w-12">{day}</span>
            ...
            <div className="relative z-10">
                ...
                <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
                  ...
                  >
                    <div className={`w-3 h-3 sm:w-4 sm:h-4 rounded-full border-2 flex items-center justify-center transition-all ${
                      isFreeTimer ? 'border-[#D7C8F3] bg-[#D7C8F3]' : 'border-white/40'
                    }`}>
                      {isFreeTimer && <div className="w-1 h-1 sm:w-1.5 sm:h-1.5 bg-white rounded-full" />}
                    </div>
                    <span className="text-white/90">{t?.dashboard?.free || 'Free'}</span>
                  </button>
                  ...
                  <select
                    ...
                    className={`px-2 sm:px-4 py-1.5 sm:py-2 bg-white/10 border border-white/20 rounded-lg text-white/90 text-xs sm:text-sm focus:outline-none focus:border-[#D7C8F3] transition-colors backdrop-blur-sm max-w-[140px] sm:max-w-none truncate ${
                      isFreeTimer || isRunning ? 'opacity-40 cursor-not-allowed' : ''
                    }`}
                  >
              ...
              <div className="flex flex-col items-center justify-center py-4 sm:py-6 md:py-8">
                <div className="relative">
                  ...
                  <svg className="w-48 h-48 sm:w-56 sm:h-56 md:w-72 md:h-72 transform -rotate-90">
                    ...
                    <circle
                      ...
                      strokeDashoffset={2 * Math.PI * 45 * (1 - getTimerProgress())}
                      className="transition-all duration-1000 ease-linear"
                      style={{
                        filter: 'drop-shadow(0 0 8px rgba(215, 200, 243, 0.8)) drop-shadow(0 0 20px rgba(215, 200, 243, 0.5)) drop-shadow(0 0 40px rgba(215, 200, 243, 0.3))',
                      }}
                    />
                    {/* Gradient definition */}
                    <defs>
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center">
                    ...
                    <span className="text-white/50 text-xs sm:text-sm mt-1 sm:mt-2 max-w-[120px] sm:max-w-none text-center truncate">
                      {isFreeTimer 
                        ...
                        : habits.find(h => h.id === selectedHabitId)?.name || (t?.dashboard?.selectHabit || 'Select a habit')
                      }
                    </span>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-4 sm:gap-6 mt-6 sm:mt-8">
                  ...
                  >
                    {isRunning ? <FiPause size={24} className="sm:w-7 sm:h-7" /> : <FiPlay size={24} className="ml-1 sm:w-7 sm:h-7" />}
                  </button>
                ...
                {!isFreeTimer && selectedHabitId && (
                  <div className="mt-4 sm:mt-6 text-center">
                    <p className="text-white/40 text-[10px] sm:text-xs uppercase tracking-wider mb-1">{t?.dashboard?.todaysProgress || "Today's Progress"}</p>
                    <p className="text-[#D7C8F3] text-base sm:text-lg font-medium">
                      {habits.find(h => h.id === selectedHabitId)?.todayValue || 0}{' '}
                      <span className="text-white/50 text-xs sm:text-sm">
                        / {habits.find(h => h.id === selectedHabitId)?.targetValue} {habits.find(h => h.id === selectedHabitId)?.unit}
                      </span>
                    </p>
                  </div>
                )}
                ...
                {isFreeTimer && (
                  <div className="mt-4 sm:mt-6 text-center">
                    <p className="text-white/40 text-[10px] sm:text-xs uppercase tracking-wider">
                      {t?.dashboard?.noHabitTracking || 'No habit tracking • Just focus'}
                    </p>
                  </div>
                )}
              </div>
